...
function switchTab(tabName) {
    ...
    const titles = {
        dashboard: 'HYDROFIT Dashboard', profile: 'My Profile', activity: 'Activities',
        attendance: 'Attendance', 'teacher-dashboard': 'Teacher Dashboard',
        movement: 'Movement Library', 'ai-assist': 'AI Exercise Guide', scheduler: 'Workout Scheduler',
        timer: 'Exercise Timer', warmup: 'Warmup Generator', injury: 'Injury Prevention Guide',
        goals: 'Goal Planner', bodyparts: 'Body Focus', calorie: 'Calorie Tracker',
        bmi: 'BMI Tracker', recovery: 'Recovery & Rest', bodytype: 'Body Type Analysis'
    };
...
async function loadProfile() {
    ...
    const profileHtml = `
        ...
        <div class="profile-info-grid">
            ...
            <div class="info-item"><label>Program</label><p>${currentUser.program}</p></div>
            ...
            <div class="info-item"><label>Subject</label><p>${currentUser.subject}</p></div>
            <div class="info-item"><label>Total Attendance</label><p>${currentUser.attendanceCount || 0}</p></div>
        </div>
    ...
    // Simple format: SchoolID|FullName|Program
    const qrData = `${currentUser.schoolId}|${currentUser.fullName}|${currentUser.program}`;
...
async function loadAttendanceTracker() {
    ...
    container.innerHTML = `
        ...
        <div class="card mt-4"><h3><i class="fas fa-chart-line"></i> Attendance Progress</h3>
            <div class="progress-bar"><div class="progress-fill" style="width: ${Math.min((currentUser.attendanceCount / 20) * 100, 100)}%"></div></div>
...
// ========================================
// QR SCANNER FOR ATTENDANCE (FIXED)
// ========================================
            ...
            if (devices && devices.length > 0) {
                ...
                await html5QrCode.start(cameraId, { fps: 10, qrbox: { width: 250, height: 250 } },
                    async (decodedText) => {
                        ...
                        // Parse QR data format: SchoolID|FullName|Program
        ...
        for (const [year, sections] of Object.entries(years)) {
            ...
            for (const [section, studentsList] of Object.entries(sections)) {
                ...
                studentsList.forEach(s => {
                    html += `<tr><td>${escapeHtml(s.originalSurname)}</td><td>${s.schoolId}</td><td><strong>${s.attendanceCount}</strong> classes</td></tr>`;
                });
